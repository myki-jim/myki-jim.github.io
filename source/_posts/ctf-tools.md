---
title: CTF工具集｜by ki
date: 2024-11-27
tags: [CTF, 工具, 网络安全]
categories: [CTF, 工具]
banner:
layout: post
---

# 工具总结

- http://esoteric.sange.fi/brainfuck/impl/interp/i.html //解析一些特殊符号组成的图形
- https://tu.sioe.cn/gj/fenjie/ //将gif拆分为png
- https://www.toolhelper.cn/EncodeDecode/EncodeDecode //简单的进制转换
- https://online-barcode-reader.inliteresearch.com/ //二维码
- http://app.xunjiepdf.com/pdf2word//pdf转换器
- http://www.keyfc.net/bbs/tools/tudoucode.aspx //佛语。。
- https://lzltool.cn/Tools/Rot13

# 相关下载

- winhex（磁盘编辑工具）
  - 下载地址：https://zhangfa.lanzoub.com/iQEARyoy2ze
- ARCHPR（压缩包破解）
  - 下载地址：https://zhangfa.lanzoub.com/iQmy8yq3tuh
- Ziperello（压缩包密码破解）
  - 下载地址：https://zhangfa.lanzoub.com/idz1Xzuvxfe
- ExeinfoPe（脱壳检测工具）
  - 下载地址：https://zhangfa.lanzoub.com/ie4Tv01uszuf
- IDAPro（逆向分析工具）
  - 下载地址：https://download.csdn.net/download/l8947943/85018706
- jetbrain学生党使用（请支持正版）
  - https://www.gwalker.cn/article-f4a2a56g519ac9529d3a3cf927b78c0f.html
- CTF小工具在线合集（多种小工具yyds，用于加解密，二维码在线处理等）
  - http://www.hiencode.com/
  - https://www.ctftools.com/down/
  - https://www.wishingstarmoye.com/
- 密码学大素数在线分解
  - http://www.factordb.com/index.php
- 蚁剑工具（注入连接后端工具）
  - https://zhangfa.lanzoub.com/iv2S801uxjti
- 御剑工具汇总（注入连接后端工具）
  - https://zhangfa.lanzoub.com/ia8XQ01uxk0f
- Stegsolve（隐写分析工具）
  - https://zhangfa.lanzoub.com/idNsd06v43sd
- ZipCenOp.jar（自动修复zip伪加密文件）
  - https://zhangfa.lanzoub.com/imhZ206wft0d
- Audacity_3.1.3.exe（用于音频分析剪辑）
  - https://zhangfa.lanzoub.com/iO2k10729zih
- QR_Research.exe（二维码扫描小工具）
  - https://zhangfa.lanzoub.com/idPZt073id4b
- 暴力破解字典库（常用于爆破用的密码库）
  - https://github.com/rootphantomer/Blasting_dictionary

# 常用MISC（杂项）工具

- Audacity （提取莫斯密码辅助工具）
- stegsolve （图片隐写分析工具）
- QR_Research （二维码工具）
- ZipCenOp.jar(验证是否为伪加密工具)
- AAPR(rar密码破解工具)
- Ziperello(zip密码破解工具)
- 7.Winhex(程序查看调试工具)
- RouterPassView(路由器配置查看工具)
- 四、常用REVERSE（逆向）工具
- ExeinfoPe （判断软件版本和是否有壳）
- IDA64位
- IDA32位
- 五、ctf常用扫描字典

（解码工具箱

zip文件的结尾以一串504B0506开始、
rar文件以C43D7B00400700结尾。
JPG文件结尾为FFD9
PNG文件 结尾为000049454E44AE426082。
Gif文件结尾为3B。

cat指令在windows中替换为type
https://www.gpsvisualizer.com/map_input //地图