...
const MagicNavbar: React.FC<MagicNavbarProps> = ({ onNavigate, activePage }) => {
        ...
        <div className="w-full h-full relative max-w-7xl mx-auto px-6 flex items-center justify-between" ref={navbarRef}>
                                      ...
                                      <div key={post.id} onClick={() => onNavigate('post', post.id)} className="flex items-center gap-3 p-3 hover:bg-[var(--glass-surface-hover)] transition-colors rounded-xl cursor-pointer group">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <span className="text-[9px] uppercase tracking-wider text-[var(--text-tertiary)] border border-[var(--glass-border)] px-1 py-0.5 rounded flex-shrink-0">{post.category}</span>
                                            </div>
            ...
            >
                ...
                <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onNavigate('home')}>
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-cyan-400 to-blue-600 shadow-[0_0_15px_rgba(34,211,238,0.5)] group-hover:scale-110 transition-transform duration-300" />
                    <span className="font-bold text-xl tracking-tight text-[var(--text-primary)] group-hover:text-[var(--accent-color)] transition-colors">LiquidOS</span>
                </div>
                ...
                <div className="flex items-center gap-3 relative">
                    ...
                    {/* More Button (Docked State) */}
                    <button 
                        onClick={() => setIsMoreMenuOpen(!isMoreMenuOpen)}
                        className={`p-2 rounded-full transition-all active:scale-95 ${isMoreMenuOpen ? 'bg-[var(--glass-surface-hover)] text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:bg-[var(--glass-surface-hover)] hover:text-[var(--text-primary)]'}`}
                    >
